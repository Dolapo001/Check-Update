...
from core.utils.email_verification import test_email_connection, send_email_with_template
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if test_email_connection():
            self.stdout.write(
                self.style.SUCCESS("✓ Email connection test passed")
            )
        else:
            self.stdout.write(
                self.style.ERROR("✗ Email connection test failed")
            )
            return
        ...
        try:
            if method == 'django':
                ...
                if result:
                    ...
                    self.stdout.write(
                        self.style.ERROR(f"✗ Failed to send template test email to {recipient_email}")
                    )
