...
class RecommendationEngine:
    ...
    def get_recommendations(user, limit=10):
        ...
        # Combine preferences
        preferred_categories = bookmarked_categories  # Add viewed_categories if available

        # Get news from preferred categories
        recommendations = News.objects.filter(
            subcategory__category__in=preferred_categories
        ).exclude(bookmarks=user).order_by('-created_at', '-views')[:limit]
